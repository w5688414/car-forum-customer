...
class SentimentProcessor(DataProcessor): # TODO NEED CHANGE2
  """Processor for the internal data set. sentence pair classification"""
  def __init__(self):
    self.language = "zh"
...
class CarForumProcessor(DataProcessor): # TODO NEED CHANGE2
  """Processor for the internal data set. sentence pair classification"""
  def __init__(self):
    self.language = "zh"
  ...
  if FLAGS.do_predict:
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, prediction) in enumerate(result):
        ...
        output_line=str(index)+'\n'
        writer.write(output_line)
        num_written_lines += 1
    output_proba(res,"albert_probabilities.tsv")
    assert num_written_lines == num_actual_predict_examples

def output_proba(result,file_name):
    with open(file_name,"w") as writer:
        output_line="label"+'\n'
        writer.write(output_line)
        for prediction in result:
            # str(prediction[0])+'\t'
            probabilities =str(prediction[0])+'\t'+str(prediction[1])+'\n'
            print(probabilities)
