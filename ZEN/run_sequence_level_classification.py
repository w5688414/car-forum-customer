...
def predict(args, model, tokenizer, ngram_dict, processor, label_list):
    ...
    with open('predict_results.txt','w') as f:
        ...
        for item in prediction:
            f.write(str(item)+'\n')
    output_proba(preds,"albert_probabilities.tsv")
    return compute_metrics(args.task_name, prediction, out_label_ids)
...
def main():
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        result = predict(args, model, tokenizer, ngram_dict, processor, label_list)
        # for key in sorted(result.keys()):
        #     logger.info("  %s = %s", key, str(result[key]))
